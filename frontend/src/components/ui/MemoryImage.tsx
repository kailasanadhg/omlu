...
export function MemoryImage({ item, alt, full = false }: { item?: MediaItem; alt: string; full?: boolean }) {
  ...
  const style = { aspectRatio: Math.max(.4, Math.min(2.5, ratio)) };
  return <div className="w-full bg-neutral-100 overflow-hidden rounded-lg flex items-center justify-center" style={style}>
